...
class Board {
  ...
  static const Board standard = Board(
    ...
    for (int i = 0; i < boardFen.length; i++) {
      ...
      if (c == '/' && file == 8) {
        ...
        if (code < 57) {
          ...
          final int square = file + rank * 8;
          final bool promoted =
              i + 1 < boardFen.length && boardFen[i + 1] == '~';
          final Piece? piece = _charToPiece(c, promoted);
  ...
  Iterable<Tuple2<Square, Piece>> get pieces sync* {
    for (final Square square in occupied.squares) {
      yield Tuple2<int, Piece>(square, pieceAt(square)!);
    }
  }
  ...
  Piece? pieceAt(Square square) {
    final Side? side = sideAt(square);
    if (side == null) {
      return null;
    }
    final Role role = roleAt(square)!;
    final bool prom = promoted.has(square);
